...
void fillNode(AvlTree *node, int key, int data, int height, AvlTree *parent, AvlTree *left, 
	AvlTree *right);
...
void rotateR(AvlTree **tree) {
	...
	if (left->right != NULL) left->right->parent = (*tree);
...
void rotateL(AvlTree **tree) {
	...
	if (right->left != NULL) right->left->parent = (*tree);
...
void addNode(AvlTree **tree, int key, int data) {
	if ((*tree) == NULL) {
		...
		fillNode((*tree), key, data, 1, NULL, NULL, NULL);
		return;
	}
	if (key < (*tree)->key) {
		if ((*tree)->left == NULL) {
			(*tree)->left = malloc(sizeof(AvlTree));
			fillNode((*tree)->left, key, data, 1, (*tree), NULL, NULL);
		...
		if ((*tree)->right == NULL) {
			...
			fillNode((*tree)->right, key, data, 1, (*tree), NULL, NULL);
